...
namespace SancionadosSAGB2025.Client.Componentes.FaltasDeServidoresPúblicosN.Components
{
	partial class FormAgregarFaltasServidoresPNG
	{
        ...
        //{
        //    // Siempre asegúrate de que todas las propiedades anidadas existan
        //    // antes de que el formulario se renderice.
        //    FaltasDeServidoresPublicosG ??= new AddFaltasDeServidoresPublicosNoGraves();

        //    // Inicializa las propiedades anidadas si son nulas
        //    FaltasDeServidoresPublicosG.DatosGenerales ??= new DatosGenerales();
        ...
        //    // ... repite para cualquier otra propiedad anidada
        //}
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
		{
			if (IsEditMode != (int)TipoVistaComponentes.Agregar)
			{
				...
				BloqueoSesiones = Enum.GetValues(typeof(SesionesFaltasDeServidoresPublicosNoGraves))
					.Cast<int>()
					.ToList();
		...
		private async Task ValidarInformacionNivelJerarquico()
		{
			...
			bool esOtro = nivel?.Clave?.Clave?.Contains("OTRO") == true;
			...
			try
			{
				...
				if (result is not null)
				{
					...
					if (CatalogosBD is not null)
					{
						...
						FaltasCometidas = CatalogosBD.FaltaCometidas!.Where(c => c.Bandera == 0 || c.Bandera == 2).ToList();
						Sexos = CatalogosBD.Sexo!;
						ListaOrigenesInvestigacion = CatalogosBD.OrigenProcedimiento!;
						TipoSancionClaves = CatalogosBD.TipoSancion!.Where(c => c.Bandera == 0 || c.Bandera == 2).ToList();
						TipoMonedas = CatalogosBD.Monedas!;
						TiposAmonestacion = CatalogosBD.TipoAmonestacion!;
					}
				}
				else
				{
