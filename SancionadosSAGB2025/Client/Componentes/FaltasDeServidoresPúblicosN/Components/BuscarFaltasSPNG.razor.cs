...
{
	partial class BuscarFaltasSPNG
	{
		[Inject] private NavigationManager Navigation { get; set; }
        [Inject] private IJSRuntime JS { get; set; }
        private SearchFaltasDeServidoresPublicosG searchFaltasDeServidoresPublicosG { get; set; } = new();
		private List<AddFaltasDeServidoresPublicosNoGraves> faltasDeServidoresPublicosGs { get; set; } = new();
		private AddFaltasDeServidoresPublicosNoGraves faltasDeServidoresPublicosGsSeleccionadas { get; set; } = new();
		private TipoEvento TiposEventos { get; set; } = TipoEvento.Principal;
		private int IdUsuario { get; set; } = 0;

		private List<AddFaltasDeServidoresPublicosNoGraves> Elements = new List<AddFaltasDeServidoresPublicosNoGraves>();
		private string _searchString;
		private bool _sortNameByLength;
        MudDataGrid<AddFaltasDeServidoresPublicosNoGraves>? dataGrid;
		...
		public async Task MostrarVistaActualizarFalta(AddFaltasDeServidoresPublicosNoGraves addFaltasDeServidoresPublicosG)
		{
			TiposEventos = TipoEvento.Actualizar;
			faltasDeServidoresPublicosGsSeleccionadas = addFaltasDeServidoresPublicosG;
		}

		public async Task MostrarVisualizacionFalta(AddFaltasDeServidoresPublicosNoGraves addFaltasDeServidoresPublicosG)
		{
		...
		public async Task BuscarFaltasServidoresPG()
		{
			try
			{
				var result = await FaltasSPNoGravesService.ObtenerFaltasSPG(searchFaltasDeServidoresPublicosG);
		...
		private Func<AddFaltasDeServidoresPublicosNoGraves, bool> _quickFilter => x =>
		{
		...
		// events
		void RowClicked(DataGridRowClickEventArgs<AddFaltasDeServidoresPublicosNoGraves> args)
		{
			_events.Insert(0, $"Event = RowClick, Index = {args.RowIndex}, Data = {System.Text.Json.JsonSerializer.Serialize(args.Item)}");
		}

		void RowRightClicked(DataGridRowClickEventArgs<AddFaltasDeServidoresPublicosNoGraves> args)
		{
			_events.Insert(0, $"Event = RowRightClick, Index = {args.RowIndex}, Data = {System.Text.Json.JsonSerializer.Serialize(args.Item)}");
		}

		void SelectedItemsChanged(HashSet<AddFaltasDeServidoresPublicosNoGraves> items)
		{
        ...
        private async Task GenerarExcel()
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("Faltas No Graves");
                ...
                await JS.InvokeVoidAsync("BlazorDownloadFile",
                    "Reporte_Faltas_No_Graves.xlsx", // Nombre de archivo mejorado
